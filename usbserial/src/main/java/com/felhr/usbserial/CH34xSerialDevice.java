...
public class CH34xSerialDevice extends UsbSerialDevice {
    ...
    public void setBreak(boolean state) {
        int ch341_break_reg = ((CH341_REG_BREAK2 << 8) & 0xffff) | CH341_REG_BREAK1;
//unstable
//        Log.i(CLASS_ID, ("ch341_break_reg:" + String.format("%x", ch341_break_reg)));
//        int reg_contents;
//        byte[] break_reg = new byte[2];
//        byte[] temp_buff = new byte[1024];
//        int ret = setControlCommandIn(CH341_REQ_READ_REG, ch341_break_reg, 0, break_reg);
//
//        if (ret != 2) {
//            Log.i(CLASS_ID, ("Expected " + "2" + " bytes, but get " + ret));
//            return;
//        }
//        Log.d(CLASS_ID, "Initial ch341 break register contents - reg1: " + String.format("%x", break_reg[0]) + ", reg2: " + String.format("%x", break_reg[1]));
//
//        if (state) {
...
//            break_reg[1] |= CH341_NBREAK_BITS_REG2;
//        }
//        Log.d(CLASS_ID, "New ch341 break register contents- reg1: " + String.format("%x", break_reg[0]) + ", reg2: " + String.format("%x", break_reg[1]));
//
//        reg_contents = get_unaligned_le16(break_reg);
//        Log.i(CLASS_ID, ("reg_contents:" + String.format("%x", reg_contents)));
//
//        ret = setControlCommandOut(CH341_REQ_WRITE_REG, ch341_break_reg, reg_contents, new byte[0]);
//        if (ret < 0) {
//            Log.i(CLASS_ID, "USB control write error with ret");
//        }
//        //Fixed
        if (state) {
            setControlCommandOut(CH341_REQ_WRITE_REG, ch341_break_reg, 0xbf3e, null);
        } else {
            setControlCommandOut(CH341_REQ_WRITE_REG, ch341_break_reg, 0xffbf, null);
        }
    }

    private int get_unaligned_le16(byte[] _p) {
        return (_p[0] | ((_p[1] << 8) & 0xffff)) & 0xffff;
    }
    ...
    private int setControlCommandOut(int request, int value, int index, byte[] data) {
        ...
        int response = connection.controlTransfer(REQTYPE_HOST_TO_DEVICE|0x40, request, value, index, data, dataLength, USB_TIMEOUT);
